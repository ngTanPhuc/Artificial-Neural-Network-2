...
protected:
    DLinkedList<Entry* >* table;  //array of DLinkedList objects  // table is a pointer points to (key, value) pairs
...
xMap<K,V>::xMap(
    ...
    // Assign the capacity and the count of the table
    this->capacity = 10;
    this->count = 0;

    // Assign the loadFactor
    this->loadFactor = loadFactor;

    // Create a new ARRAY of DLinkedList (think a hash table as an array of type DLinkedList)
...
V xMap<K,V>::put(K key, V value){
    ...
    V retValue = value;
    //YOUR CODE IS HERE    
    if (!table[index].empty()) { // If the DLL at index  is not empty
        // Traverse the DLL, looking for pair with the same key
        bool isSameKey = false;
        
        for (auto it = table[index].begin(); it != table[index].end(); ++it) {
            // Since we've declared XMap as a friend class to Entry class, we can access the private attributes
            if (keyEQ(key, (*it)->key)) {
                isSameKey = true;
                retValue = (*it)->value;
                (*it)->value = value;
        ...
        if (!isSameKey) { // Create a new pair that will be added to the DLinkedList
            Entry* newPair = new Entry(key, value);  // !Entry
            table[index].add(newPair);
        }
    } else {  // If not, create a new pair that will be added to the DLinkedList
...
V xMap<K,V>::remove(K key,void (*deleteKeyInMap)(K)){
    int index = hashCode(key, capacity);
    V retValue;  // !V retValue = value;
    //YOUR CODE IS HERE
    for (auto it = table[index].begin(); it != table[index].end(); ++it) {
        // Since we've declared XMap as a friend class to Entry class, we can access the private attributes
        if (keyEQ(key, (*it)->key)) { // If found the key
            retValue = (*it)->value;  // Backup the value to return

            if (deleteKeyInMap != nullptr) {  // Free the key if deleteKeyInMap is not null
                deleteKeyInMap((*it)->key);
            }

            // Remove the Entry from the list and free the memory of the Entry
...
bool xMap<K,V>::remove(K key, V value, void (*deleteKeyInMap)(K), void (*deleteValueInMap)(V)){
    ...
    for (auto it = table[index].begin(); it != table[index].end(); ++it) {
        // Since we've declared XMap as a friend class to Entry class, we can access the private attributes
        if (keyEQ(key, (*it)->key) && valueEQ(value, (*it)->value)) { // If found the key
            foundKeyValue = true;

            if (deleteKeyInMap != nullptr) {  // Free the key if deleteKeyInMap is not null
                deleteKeyInMap((*it)->key);
            }
            if (deleteValueInMap != nullptr) {  // Free the value if deleteValueInMap is not null
...
bool xMap<K,V>::containsValue(V value){
    ...
    for (int i = 0; i < capacity; i++) {
        if (!table[i].empty()) {  // If the DLL at the index i is not empty
            for (auto it = table[i].begin(); it != table[i].end(); ++it) {
                if (valueEQ(value, (*it)->value)) {
                    return true;
                }
            }
        } else {
 ...
 *      i.e., the maximum number of entries does not exceed "loadFactor*capacity" 
    !This needs to be called when adding a new pair
 */
template<class K, class V>
...
void xMap<K,V>::copyMapFrom(const xMap<K,V>& map){
    // removeInternalData();  // !Check if needed to be delete since it might cause errors
    
    this->capacity = map.capacity;
    this->count = 0;
    this->table = new DLinkedList<Entry*>[capacity];  // Create a new table
